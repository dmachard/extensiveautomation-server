...
import Core.RestServerInterface as RSI
...
swagger_info = [
                    ...
                    ( "title", "Swagger %s" % Settings.get( 'Server', 'name') ),
                ]
swagger_schemes = [ 
                    "https"
                  ]
...
py_tab = 4
py_decorators = findDecorators(module=RSI)
...
# find defintions
swagger_defs = []
py_decorators = findDecorators(module=RSI, deco="_to_yaml_defs")
...
swagger_tags = [ ]
py_decorators = findDecorators(module=RSI, deco="_to_yaml_tags")
if len(py_decorators): 
    for (c,fcs) in py_decorators:
        obj = getattr(RSI, c)
        for f in fcs:
