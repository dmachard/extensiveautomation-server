...
def createTestExecutable( dataTest, 
    ...
    if dataTest["test-extension"] == "tgx":
        return createTestGlobal( dataTest, userName, testName, trPath,
                                ...
                                defaultAdapter, userId, projectId, subTEs, 
...
""")
	...
	ParametersHandler.addParameters( parametersId=TLX.instance().mainScriptId, parameters=%s)\n""" % parameters )
    # te.append( """
	# ParametersHandler.addAgents( agentsId=TLX.instance().mainScriptId, agents=%s)\n""" % agents )
    te.append( """
	ParametersHandler.addDescriptions( descriptionsId=TLX.instance().mainScriptId, descriptions=%s)\n""" % descriptions )
    te.append( """
	ParametersHandler.addRunningAgents( agents=%s)\n""" % runningAgents )
...
def createTestSuite (	dataTest, 
    ...
    te.append("""
TestAdapter.setMainPath(sutPath=root)
TestProperties.initialize(parameters=%s, descriptions=%s, agents=%s, 
                        parametersShared=%s, runningAgents=%s)""" % (parameters, descriptions, agents, parametersShared, runningAgents) )
...
def createTestUnit (	dataTest, 
    ...
    te.append("""
TestAdapter.setMainPath(sutPath=root)
TestProperties.initialize(parameters=%s, descriptions=%s, agents=%s, parametersShared=%s, runningAgents=%s)""" % (parameters, descriptions, agents, parametersShared, runningAgents) )
