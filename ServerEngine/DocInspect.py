...
def print_error(err):
    """
    Easy function to print error
    """
    sys.stderr.write("%s\n" % err)

def print_msg(err):
    """
    Easy function to print message
    """
    sys.stdout.write("%s\n" % err)
...
def describeModule( module, classes, descr='', removeVersion=False ):
    """
    Describe the module object passed as argument including its classes and functions
    ...
    desc= { 'name': moduleName[1] , 'realname': fullModuleName, 'classes': ret, 'type': 'module', 'desc': descr }
...
def describePackage( package, modules, descr='' ):
    """
    Describe the python package object passed as argument including its classes and functions
    ...
    """
    ret = []
    desc = { 'name': package.__name__ , 'modules': ret , 'type': 'package', 'desc': descr }
...
def inspectLibrary( package, modules):
    """
    Describe the python package object passed as argument including its classes and functions
    ...
    for name in dir(package):
        for m in modules:
            ...
            if n == name:
                obj = getattr(package, name)
                if inspect.ismodule(obj):
                    ret.append( describeModule(obj, c, d, removeVersion=True) )
    return ret
