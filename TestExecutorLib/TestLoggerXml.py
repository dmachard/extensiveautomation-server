...
class TestLoggerXml:
    ...
    def __init__ (self, task_uuid, path, name, user_,testname_, id_, 
        ...
        if sys.version_info > (3,):
            self.fd_logs = open('%s/%s' % (self.__path, self.__filename), 'a+', 1)
        else:
            self.fd_logs = open('%s/%s' % (self.__path, self.__filename), 'a', 0)

        if sys.version_info > (3,):
            self.fd_hdr = open('%s/%s' % (self.__path, self.__filename_hdr), 'a+', 1)
        else:
            self.fd_hdr = open('%s/%s' % (self.__path, self.__filename_hdr), 'a', 0)
        
        self.regex = re.compile("(\d+-\d+-\d+)")
    def getPath(self):
        """
        Return path
        """
    ...
    def to_notif(self, value = '', testInfo={}):
        ...
        try:
            ...
            if not TestSettings.getInt( 'Tests_Framework', 'test-in-background' ):
