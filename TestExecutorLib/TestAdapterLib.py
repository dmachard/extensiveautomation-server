...
__HELPER__ = [
                ('Adapter', ['__init__', 'getAdapterId', 'testcase', 'privateGetFile', 'privateAddFolder', 'privateGetPath', 'privateSaveFile', 'privateAppendFile', 'setRunning', 'unsetRunning', 'stopRunning', 'received', 'logSentEvent', 'logRecvEvent', 'info', 'error', 'warning', 'debug', 'onRun', 'onReset', 'receivedNotifyFromAgent', 'receivedErrorFromAgent', 'receivedDataFromAgent']), 
                ('State', ['__init__', 'set', 'get']), 
                ('Timer', ['__init__', 'setDisable', 'setEnable', 'setDuration', 'start', 'stop', 'restart'])
             ]
...
class Adapter(threading.Thread):
    ...
    def received_thread(self, towatch, AND, XOR):
        """
        """
            ...
            if not self.queue.empty():      
                ...
                if isinstance(evtList, list):
                    ...
                    try:
                        ...
                        for i in xrange(len(towatch)):
                            ret, tpl = TestTemplatesLib.comparePayload( payload=evtList, 
                                                                        tpl=towatch[i].get(), 
                                                                        debug=self.debug ) 
                            if ret:
                                allresults.append( (i, False, evt))
                                if self.__showEvts:
                                    TLX.instance().log_match_stopped( fromComponent = componentName, dataMsg = (evtList,tpl), 
                                ...
                                if self.__showEvts:
                                    TLX.instance().log_match_info( fromComponent = componentName, dataMsg = (evtList,tpl) , 
                ...
                if AND:
                    if len(allresults) == nb_towatch:
                        evts = []
                        for r in allresults:
                            _, _, revt = allresults[0]
                            evts.append( revt )
                        if len(evts) >1:
                            self.received_result = evts
                        else:
                            self.received_result = evts[0]
                            
                        self.received_running = False
                        self.received_available.set()
                        
                # xor condition
                if XOR:
                    if len(allresults) == 1:
                        _, _, revt = allresults[0]
                        self.received_result = revt
    ...
    def received_old (self, expected, timeout, AND=True, XOR=False):
        """
        ...
        # thread = threading.Thread(target=self.received_thread, args=(towatch, AND, XOR) )
        # thread.start()
    
        # self.received_available.wait(timeout=timeout_float)
        # self.received_running=False
        
        # if self.received_result is None:
            # if self.__showEvts:
    ...
    def enqueueEvent_old (self, event):
        """
        Enqueue event
        ...
        if not self.received_running:
            ...
            for i in xrange(len(self.received_template)):
                ...
                if success:
                    ...
                    if self.__showEvts:
                        TLX.instance().log_match_stopped( fromComponent = componentName, dataMsg = (event.get(),tpl), 
                                                          tcid = self.testcaseId, font = 'italic', 
                                                          index=i,  fromlevel=self.getFromLevel(), 
                                                          tolevel=LEVEL_USER, testInfo=self.__testcase.getTestInfo())
                    ...
                    if self.__showEvts:
                        TLX.instance().log_match_info( fromComponent = componentName, dataMsg = (event.get(),tpl) , 
                                                       tcid = self.testcaseId, font = 'italic', index=i, 
                                                       fromlevel=self.getFromLevel(), tolevel=LEVEL_USER, 
                                                       testInfo=self.__testcase.getTestInfo() )
