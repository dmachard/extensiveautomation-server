...
class NMAP_01(TestCase):
	...
	def prepare(self):
		self.NMAP = SutAdapters.System.Nmap(parent=self, name=None, debug=input('DEBUG'), shared=False, agentSupport=False, agent=None)
	...
	def definition(self):
		...
		if self.step1.isEnabled():
			...
			self.NMAP.execute(cmd="www.extensivetesting.org")
			event = self.NMAP.hasReceivedEvent(expected=TestOperators.Contains(needle="(1 host up) scanned"), timeout=input('TIMEOUT'))
			self.info( event.get("NMAP", "output") )
...
<testexecution><![CDATA[
NMAP_01(suffix=None).execute()]]></testexecution>
