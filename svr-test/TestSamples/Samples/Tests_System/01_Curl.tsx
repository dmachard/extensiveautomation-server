...
class CURL_01(TestCase):
	...
	def prepare(self):
		self.CURL = SutAdapters.System.Curl(parent=self, name=None, debug=input('DEBUG'), shared=False, agentSupport=False, agent=None)
	...
	def definition(self):
		...
		if self.step1.isEnabled():
			...
			self.CURL.execute(cmd="-s -v -D - https://www.extensivetesting.org")
			event = self.CURL.hasReceivedEvent(expected=TestOperators.Contains(needle="200 OK"), timeout=input('TIMEOUT'))
			self.info( event.get("CURL", "output") )
...
<testexecution><![CDATA[
CURL_01(suffix=None).execute()]]></testexecution>
