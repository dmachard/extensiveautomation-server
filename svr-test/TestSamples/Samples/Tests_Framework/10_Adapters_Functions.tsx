...
#		t = threading.Thread(target=self.__test)
#		t.start()
#		
#	def __test(self):
#		import threading

		l = SutAdapters.LDAP.Client(parent=self, ip="wwww.goog.fr", dn="@string", password="@string", port=389, name=None, 
										debug=False, shared=False, agentSupport=False, agent=None)
		l.connect()
...
class IMPORT_GENERIC_01(TestCase):
	def description(self):
		# testcase description
		self.setPurpose(purpose="Testcase sample")
	...
	def definition(self):
		...
		if self.step1.isEnabled():
			...
			self.info( "generic=%s" % SutAdapter.getVersionGeneric() )
			self.info( "generic=%s" %  SutLibrary.getVersionGeneric() )
			
			self.info( "default=%s" % SutAdapter.getVersion() )
			self.info( "default=%s" %  SutLibrary.getVersion() )
		
			A = SutAdapters.test.Adapter(parent=self, debug=False, name=None, shared=False)
			A.helloWorld()
	
			B = SutAdapters.Generic.Dummy.Adapter(parent=self, debug=False, name=None, shared=False)
			B.helloWorld()
...
#STORAGE_03(suffix=None).execute()
#IMPORT_GENERIC_01(suffix=None).execute()]]></testexecution>
