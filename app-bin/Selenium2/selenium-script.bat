...
set SELENIUM_JAR="%~dp0\selenium-server-standalone.jar"
...
echo +++ using: %PARMS% -jar %SELENIUM_JAR% %SELENIUM_COMMAND%
java %PARMS% -jar %SELENIUM_JAR% %SELENIUM_COMMAND%
GOTO FINALLY
...
echo +++ trying to run selenium
"%JAVA_HOME%\bin\java.exe" -jar %SELENIUM_JAR% %SELENIUM_COMMAND%
GOTO FINALLY
