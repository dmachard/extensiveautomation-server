...
class InteropPlugin(object):
    ...
    def logRequest(self, msg, details, raw=""):
        ...
        tpl.addRaw(raw)
        try:
            TLX.instance().log_snd(msg, tpl.getEvent(), tpl.type(), TestExecutor.TC, self.tcparent.getId(), 
                                    fromlevel=FROM_LEVEL, tolevel=self.__class__.__name__.upper(), 
    ...
    def logResponse(self, msg, details, raw=""):
        ...
        tpl.addRaw(raw)
        try:
            TLX.instance().log_rcv(msg, tpl.getEvent(), tpl.type(), TestExecutor.TC, self.tcparent.getId(), 
                                    fromlevel=self.__class__.__name__.upper(), tolevel=FROM_LEVEL, 
                                    testInfo=self.tcparent.getTestInfo())
        except Exception as e:
