...
class TemplateLayer(object):
    ...
    def __init__(self, name):
        ...
        """
        if sys.version_info > (3,):
            if isinstance(name, bytes): name = base64.encodebytes(name).decode("utf8")
            
        if not ( isinstance(name, str) \
                or isinstance(name, unicode) \
                or isinstance(name, int) \
                or isinstance(name, TestOperatorsLib.Any) \
                or isinstance(name, TestOperatorsLib.Startswith) \
                or isinstance(name, TestOperatorsLib.NotStartswith) \
                or isinstance(name, TestOperatorsLib.Endswith) \
                or isinstance(name, TestOperatorsLib.NotEndswith) \
                or isinstance(name, TestOperatorsLib.Contains) \
                or isinstance(name, TestOperatorsLib.NotContains) \
                or isinstance(name, TestOperatorsLib.GreaterThan) \
                or isinstance(name, TestOperatorsLib.LowerThan) \
                or isinstance(name, TestOperatorsLib.RegEx) \
                or isinstance(name, TestOperatorsLib.NotRegEx) \
                or isinstance(name, TestOperatorsLib.NotGreaterThan) \
                or isinstance(name, TestOperatorsLib.NotLowerThan) ):
    ...
    def addKey(self, name, data):
        ...
        if sys.version_info > (3,):
            if isinstance(name, bytes): name = base64.encodebytes(name).decode("utf8")
            if isinstance(data, bytes): data = base64.encodebytes(data).decode("utf8")
        ...
        if not ( isinstance(data, TemplateLayer) \
                or  isinstance(data, int) \
                or  isinstance(data, str) \
                or  isinstance(data, unicode) \
                or isinstance(data, TestOperatorsLib.Any) \
                or isinstance(data, TestOperatorsLib.Startswith)\
                or isinstance(data, TestOperatorsLib.NotStartswith) \
                or isinstance(data, TestOperatorsLib.Endswith) \
                or isinstance(data, TestOperatorsLib.NotEndswith)\
                or isinstance(data, TestOperatorsLib.Contains) \
                or isinstance(data, TestOperatorsLib.NotContains) \
                or isinstance(data, TestOperatorsLib.GreaterThan) \
                or isinstance(data, TestOperatorsLib.LowerThan) \
                or isinstance(data, TestOperatorsLib.RegEx) \
                or isinstance(data, TestOperatorsLib.NotRegEx) \
                or isinstance(data, TestOperatorsLib.NotGreaterThan) \
                or isinstance(data, TestOperatorsLib.NotLowerThan) ):
...
def is_ops(k):
    ...
    @type k:
    """
    if isinstance(k, TestOperatorsLib.Any) \
        or isinstance(k, TestOperatorsLib.Startswith) \
        or isinstance(k, TestOperatorsLib.NotStartswith) \
        or isinstance(k, TestOperatorsLib.Endswith) \
        or isinstance(k, TestOperatorsLib.NotEndswith) \
        or isinstance(k, TestOperatorsLib.Contains) \
        or isinstance(k, TestOperatorsLib.NotContains) \
        or isinstance(k, TestOperatorsLib.GreaterThan) \
        or isinstance(k, TestOperatorsLib.LowerThan) \
        or isinstance(k, TestOperatorsLib.NotGreaterThan) \
        or isinstance(k, TestOperatorsLib.NotLowerThan) \
        or isinstance(k, TestOperatorsLib.RegEx) \
        or isinstance(k, TestOperatorsLib.NotRegEx):
        return True
    else:
        return False
