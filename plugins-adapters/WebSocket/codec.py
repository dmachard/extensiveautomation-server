...
WEBSOCKET_OPCODE_UNKNOWN_STR = "Unknown"

WEBSOCKET_OPCODE_TEXT		    = 1
WEBSOCKET_OPCODE_TEXT_STR		= "Text"
WEBSOCKET_OPCODE_BINARY		  = 2
WEBSOCKET_OPCODE_BINARY_STR = "Binary"
WEBSOCKET_OPCODE_CLOSE	    = 8
WEBSOCKET_OPCODE_CLOSE_STR  = "Close"
WEBSOCKET_OPCODE_PING		    = 9
WEBSOCKET_OPCODE_PING_STR   = "Ping"
WEBSOCKET_OPCODE_PONG		    = 10
WEBSOCKET_OPCODE_PONG_STR   = "Pong"
...
class Codec(object):
	...
	def createSecWsKey(self):
		...
		try:
			...
			self.error( 'unable to create sec key: %s' % e )			
		return encoded
	...
	def getHeaderForwardedFor(self, request):
		...
		for hdr in request.splitlines()[1:]:
			k, v = hdr.split(':', 1)
			if k.lower().strip() == 'x-forwarded-for':
		...
		try:
			for hdr in request.splitlines()[1:]:
				...
				if k.lower().strip() == 'upgrade' and v.lower().strip() == 'websocket':
					hdrUpgrade = True
				if k.lower().strip() == 'connection' and 'upgrade' in v.lower().strip():
					hdrConnection = True
				if k.lower().strip() == 'sec-websocket-key':
					...
					if keyLength != 16:
						self.error( 'bad key length: %s' % keyLength )	  
						wsKey = False
				if k.lower().strip() == 'sec-websocket-version':
					if v.lower().strip() == "%s" % WEBSOCKET_VERSION:
						wsVersion = True
					
		except Exception as e:
			self.error( 'unable to check req headers: %s' % e )						
	...
	def decodeWsData(self, buffer):
		...
		try:
			try:
				...
				self.debug( "ws header: %s, %s, %s, %s, %s, %s, %s" % (fin, rsv1, rsv2, rsv3, opcode, has_mask, length) )
				...
				if length == 126:
					try:
						...
						if fin:
							if len(buffer) < hdrs_len+hdrs_extended_len+length_ext:
								self.debug( "data extended, need more data (%s/%s)" % (len(buffer), length_ext) )
								left = buffer # need more data
								needMore = True
							else:
					...
					if fin:
						if len(buffer) < hdrs_len+length:
							self.debug( "fin=1 but insufficient data in buffer, need more (%s/%s)" % (len(buffer), length) )
				...
				if not needMore:
					...
					tpl = templates.ws(fin=fin, mask=has_mask, rsv1=rsv1, rsv2=rsv2, rsv3=rsv3, opcode=opcode, 
															data=payload, data_length=len(payload), more=templates.received() )
					tpl.addRaw( buffer )
		except Exception as e:
			self.error( 'unable to decode ws data: %s' % e )
		return (payload, left, needMore, tpl)
		
	def encodeWsData(self, tpl):
		"""
		Encode ws message
		...
		try:
			fin = tpl.getInt('fin')
			rsv1 = tpl.getInt('rsv1')
			rsv2 = tpl.getInt('rsv2')
			rsv3 = tpl.getInt('rsv3')
			opcode = tpl.getInt('opcode')
			mask = tpl.getInt('mask')
			data = tpl.get('data')
			if data is None:
				data = ''
			length = len(data)
			...
			self.error( 'unable to encode ws data: %s' % e )
		return ''.join(ws_packet)
...
# 
#	def encodePong(self, data='abcdef'):
#		"""
